...
CREATE TABLE `sys_user` (
  ...
  `sys_user_phone` int(12) NOT NULL COMMENT '用户手机号码',
  `sys_user_sex` varchar(6) NOT NULL COMMENT '用户性别',
  `sys_user_qq` mediumint(9) NOT NULL COMMENT '用户QQ号码',
  `sys_user_email` varchar(64) NOT NULL COMMENT '用户EMAIL地址',
  `sys_user_address` varchar(255) NOT NULL COMMENT '用户地址',
  `sys_user_mark` mediumint(9) NOT NULL COMMENT '用户积分',
  `sys_user_rank_id` tinyint(3) NOT NULL COMMENT '用户等级',
  `sys_user_last_login_ip` varchar(15) NOT NULL COMMENT '用户上一次登录IP地址',
  `sys_user_birthday` datetime(6) NOT NULL COMMENT '用户生日',
  `sys_user_description` varchar(255) NOT NULL COMMENT '自我描述',
  `sys_user_image_url` varchar(255) NOT NULL COMMENT '用户头像存储路径',
  `sys_user_school` varchar(255) NOT NULL COMMENT '毕业学校',
  `sys_user_register_time` datetime(6) NOT NULL COMMENT '用户注册时间',
  `sys_user_register_ip` varchar(15) NOT NULL COMMENT '用户注册时IP地址',
  `sys_user_last_update_time` datetime(6) NOT NULL COMMENT '用户上次更新博客时间',
  `sys_user_weibo` varchar(255) NOT NULL COMMENT '用户微博',
  `sys_user_blood_type` char(3) NOT NULL COMMENT '用户血型',
  `sys_user_says` varchar(255) NOT NULL COMMENT '用户语录',
  `sys_user_lock` tinyint(3) NOT NULL COMMENT '是否锁定，0为不锁定，1为锁定',
  `sys_user_freeze` tinyint(3) NOT NULL COMMENT '是否冻结，0为不冻结，1为冻结',
  `sys_user_power` varchar(255) NOT NULL COMMENT '拥有权限',
